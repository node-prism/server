...
function createRouteHandlers(module, middleware, route, core) {
    log({ level: LogLevel.DEBUG, scope: "http" }, route);
    if (module.default && typeof module.default === "function") {
        core.app.all(route, ...middleware, wrapHandler(module.default, route));
        log({ level: LogLevel.DEBUG, scope: "http" }, `${route} | method: any -> export default`);
    }
    ["get", "put", "post", "patch", "del", "options"].forEach((method) => {
        if (module[method] && typeof module[method] === "function") {
            ...
            core.app[method === "del" ? "delete" : method](route, [...middleware, ...methodMiddleware], wrapHandler(module[method], route));
            log({ level: LogLevel.DEBUG, scope: "http" }, `${route} | method: ${method === "del" ? "DELETE" : method.toUpperCase()} -> export "${module[method].name}"`);
        }
    });
}
export default async function createHTTPHandlers(core, rootDir) {
    ...
    for (const filename of sorted) {
        ...
        log({ level: LogLevel.DEBUG, scope: "http" }, `route ${route} (${filename})`);
